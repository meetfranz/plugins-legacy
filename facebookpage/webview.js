...
module.exports = (Franz, options) => {
  const getInputsByValue = function (value) {
    ...
    for (var x = 0; x < allInputs.length; x++) {
      if(allInputs[x].getAttribute("href")) {
  ...
  const getMessages = () => {
    ...
    let unreadMessage = 0;
    if (messageParent[0] && 
        messageParent[0].children[0] && 
        messageParent[0].children[0].children[1]) 
    {
      unreadMessage = messageParent[0].children[0].children[1].innerText;
    }
